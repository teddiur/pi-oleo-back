...
from models import collector, donator
from db.database import get_db, create_tables, drop_tables
...
@app.post("/create_donator/", response_model=DonatorResponse)
...
@app.post("/create_collector/", response_model=UserResponse)
async def create_user(userRequest: UserRequest, db: Session = Depends(get_db)):
    existing_user = db.query(User).filter(User.email == userRequest.email).first()
...
async def protected_resource(current_user: UserResponse = Depends(get_current_user)):
    ...
    return {
        "msg": "Parabéns! Você acessou o endpoint secreto e agora vai descobrir quem vai pagar o próximo rodízio do Habibão!",
